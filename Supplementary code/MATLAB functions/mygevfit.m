function [parmhat,parmci] = mygevfit(x,alpha,options)
...
objFunction = @(parameters) negloglike(parameters, x) + ...
    100*(sum(gevpdf(linspace(-1, 1e-6, 1000), parameters(1), exp(parameters(2)), parameters(3)))) + ...
    (parameters(1) < 0) * 1000;
...
[parmhat,~,err,output] = fminsearch(objFunction,parmhat,options);
